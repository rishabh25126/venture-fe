...
const updates = [
  {
    ...
    content: "This section remains a placeholder until structured update feeds are backed by the API.",
  },
]
...
export default function InvestorBusinessDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const portfolioEntry = useMemo(
    () => portfolioData?.find((entry: any) => entry.business?.slug === resolvedParams.id),
        ...
        {isLoading || !business ? (
          ...
          <>
            ...
            {activeTab === "Overview" && (
              <div className="space-y-8">
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    { label: "ARR", value: business.metrics?.revenueRange || "TBD", icon: TrendingUp },
                    { label: "Customers", value: business.metrics?.userBase || "TBD", icon: Users },
                    { label: "Runway", value: business.metrics?.runway || "TBD", icon: Calendar },
                    { label: "Your Investment", value: formatCurrency(portfolioEntry.investedAmount), icon: Building2 },
                  ].map((metric) => (
                ...
                <div className="gradient-card rounded-xl border border-border p-6">
                  <h2 className="text-lg font-semibold text-foreground mb-4">About {business.name}</h2>
                  <p className="text-muted-foreground leading-relaxed mb-6">{business.description || "Business profile details will be updated soon."}</p>
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      <h3 className="font-medium text-foreground mb-2">The Problem</h3>
                      <p className="text-sm text-muted-foreground">{business.problem || "Problem statement pending."}</p>
                    </div>
                    <div>
                      <h3 className="font-medium text-foreground mb-2">Our Solution</h3>
                      <p className="text-sm text-muted-foreground">{business.solution || "Solution details pending."}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {activeTab === "Financials" && (
              <div className="gradient-card rounded-xl border border-border p-6">
                <h3 className="text-lg font-semibold text-foreground mb-4">Financial Access</h3>
                <p className="text-muted-foreground">Structured financial dashboards are still being wired. Current business metrics and invested amount are available in Overview and Cap Table.</p>
              </div>
            )}
              ...
              <div className="space-y-6">
                {updates.map((update, index) => (
                  <div key={index} className="gradient-card rounded-xl border border-border p-6">
                    ...
                    <h3 className="text-lg font-semibold text-foreground mt-3 mb-3">{update.title}</h3>
            ...
            {activeTab === "Cap Table" && (
              <div className="grid lg:grid-cols-2 gap-8">
                <div className="gradient-card rounded-xl border border-border p-6">
                  <h3 className="text-lg font-semibold text-foreground mb-6">Investor Equity Distribution</h3>
                  <div className="h-[280px] relative">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={capTableData} cx="50%" cy="50%" innerRadius={80} outerRadius={120} paddingAngle={2} dataKey="value">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                      <div className="text-center">
                        <p className="text-3xl font-bold text-foreground">{portfolioEntry.equityPercentage || 0}%</p>
                ...
                <div className="gradient-card rounded-xl border border-border p-6">
                  <h3 className="text-lg font-semibold text-foreground mb-6">Participants</h3>
                    ...
                    {[...ownerRows, ...investorRows].map((holder, index) => (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">{holder.type}{holder.email ? ` • ${holder.email}` : ""}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-semibold text-foreground tabular-nums">{holder.percentage}</p>
                          <p className="text-xs text-muted-foreground tabular-nums">{holder.type === "Investor" ? `${holder.shares} shares • ${holder.investedAmount}` : holder.investedAmount}</p>
                        </div>
                      </div>
                    ))}
                  </div>
